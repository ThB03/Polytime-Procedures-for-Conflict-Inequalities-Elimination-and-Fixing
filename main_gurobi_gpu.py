...
def run_analysis_gurobi(filepath: str, tag: str, max_vars: int, results_dir: str, has_elim: bool, has_fix: bool, device: str = "cuda"):
    ...
    dist_all = {}
    pred_all = {}
    backend = mgpu.choose_backend(device)
    ...
    start_bfs = time.time()
    for node in G.nodes:
        dist, pred = mgpu.derive_dist_pred_for_source(G, H, node, backend=backend)
        dist_all[node] = dist
        pred_all[node] = pred
    if backend == "cuda":
        torch.cuda.synchronize()
    ...
    if has_fix or has_elim or has_conf:
        ...
        start_bfs = time.time()
        for node in G.nodes:
            dist, pred = mgpu.derive_dist_pred_for_source(G_aug, H_aug, node, backend=backend)
            dist_aug[node] = dist
            pred_aug[node] = pred
        if backend == "cuda":
            torch.cuda.synchronize()
