...
def create_conflict_graph(model: Model, parts: int):
    """Build the global conflict graph and return partitioned subgraphs."""
    ...
    parts = max(1, parts)
    graph_count = graph.number_of_nodes()
    if parts == 1 or graph_count == 0:
        return graph, [graph]

    partitions = []
    try:
        _, node_groups = nxmetis.partition(graph, min(parts, graph_count))
...
def analyze_model(model: Model, tag: str, has_fix: bool = True, has_elim: bool = True, partition_threshold: int = 15000):
    # Step 1: Build conflict graph (optionally partitioned)
    threshold = max(1, partition_threshold)
    partition_count = (len(model.vars) // threshold) + 1
    full_graph, partitions = create_conflict_graph(model, partition_count)
    n = full_graph.number_of_nodes()
    m = full_graph.number_of_edges()
    if not partitions:
    ...
    for i in range(0,3):
        ...
        for subgraph in partitions:
            ...
            ae, conflictTime = add_conflict(subgraph, dist_all, stat)
            add_conflicts(subgraph, ae)

            if has_fix:
                f0, f1, fixingTime = fix(subgraph, dist_all)
                total_fixing_time += fixingTime
                total_F0 |= f0
                total_F1 |= f1
            if has_elim:
                de, ie, eliminationTime = eliminate(subgraph, dist_all, pred_all)
            ...
            # Step 4: ANS methods (restricted to partition variables)
            partition_vars = list({varname(node) for node in subgraph.nodes})
            if not partition_vars:
                continue

            nem_conflicts, nem_eliminations, nem_fixing, nem_preprocess = ANS(subgraph, partition_vars)
...
def run_analysis(filepath: str, tag: str, max_vars: int, results_dir: str, has_elim: bool, has_fix: bool, partition_threshold: int):
    ...
    (fixing_quantity, fixing_time,
     ...
     conflict_quantity, conflict_time) = analyze_model(
        model,
        tag,
        has_elim=has_elim,
        has_fix=has_fix,
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=15000,
        help="Maximum number of variables per partition (controls how many partitions are created)."
    )
    ...
    for run_id, run_config in config.items():
        ...
        try:
            ...
            run_analysis(filepath, tag, args.max_vars, results_dir, has_elim, has_fix, partition_threshold)
